...
public function agregarPersona(){
 ...
 $tipodoc= $this->input->post("tipoci");
 $nrodoc= $this->input->post("nro-ci");;
 $nombre=$this->input->post("txt-per-nombre");
 $apellido= $this->input->post("txt-per-apellido"); 
 $fecha= $this->input->post("fecha-nac")=="" ? '1777-07-07' : 
 $this->input->post("txt-per-fecha");
 $lugnac= $this->input->post("lugnac"); 
 
 $nacio= $this->input->post("txt-per-nacio");
 $nacio_orig= $this->input->post("txt-per-nacio-ori");
 $arr_nacio1= $this->Referenciales_model->nacionalidades( $nacio );
  $arr_nacio2= $this->Referenciales_model->nacionalidades( $nacio_orig);
 
  $nacio= $arr_nacio1[0]['idnacio'];
 $nacio_orig= $arr_nacio2[0]['idnacio'];
  
 $sexo= $this->input->post("r_sexo");
 
 $profe= $this->input->post("profesion");
 $arr_profe= $this->Referenciales_model->profesiones( $profe);
 $profe= $arr_profe[0]['idprofesiones'];
 
 $estadoc=$this->input->post("estado-civil");
 $domicilio=$this->input->post("domicilio");
 $barrioc=$this->input->post("barriociu");
 $telefono=$this->input->post("telefono");
 $celular=$this->input->post("celular");
 $padrec=$this->input->post("ci-padre");
 $padren=$this->input->post("nom-padre");
 $padrea=$this->input->post("ape-padre");
 $madrec=$this->input->post("ci-madre");
 $madren=$this->input->post("nom-madre"); 
 $madrea=$this->input->post("ape-madre");
 $conyuc= $this->input->post("ci-conyuge");
 $conyun=$this->input->post("nom-conyuge");
 $conyua=$this->input->post("ape-conyuge");
 $docu_orig= $this->input->post("docu-ori");
 $orig= $this->nativesession->get("orig_datos");
 $fechagra= $this->Utilidades_model->getTodayDate();
 $usua=$this->nativesession->get("usr");
 $depend=$this->nativesession->get("depend");
 $orig_actua= 'INFORMATICA';$fecha_actua= $this->Utilidades_model->getTodayDate();
	 
  
             
$sql= "insert into personas( tipodoc, nrodoc, nombre, apellido, fechanac, lugnac, nacio, 
nacio_orig, sexo,profesion,  estado_civil,domicilio, barrio_ciudad, telefono, celular,
padre_cedul,padre_nombre,padre_apelli,madre_cedul,madre_nombre,madre_apelli,
conyu_cedul,conyu_nombre,conyu_apelli,
docu_origen, orig_datos, fechagra, usuario, depend_carga, nroci,orig_actua, fecha_actua ) 
values(
'$tipodoc','$nrodoc','$nombre','$apellido','1777-07-07','$lugnac',$nacio,$nacio_orig,
'$sexo',$profe,'$estadoc','$domicilio','$barrioc','$telefono','$celular',
'$padrec','$padren','$padrea',
'$madrec','$madren','$madrea',
'$conyuc','$conyun','$conyua',
'$docu_orig','$orig','$fecha_gra','$usua','$depend',$nrodoc,'$orig_actua','$fecha_actua')";    

$retorno= array();
if($this->DB1->query( $sql)){
/**************UPLOAD IMAGE************/
/*$this->load->model("Foto_model");
$this->Foto_model->setCI_Foto( $nrodoc );
$datos= $this->Foto_model->configurarFoto();
$retorno['foto']= $datos; */
$retorno['nombre']= $nombre." ".$apellido;
}else{
    $retorno= $this->DB1->error();
}
...
public function custom_datos_persona($fields, $nrodoc){
    $campos= implode(",", $fields);
    $sql= "select $campos from personas where nrodoc=$nrodoc";
...
$sql="select a.padre_cedul,a.padre_nombre, a.padre_apelli,a.madre_cedul,a.madre_nombre,a.madre_apelli,
  a.conyu_cedul,a.conyu_nombre,a.conyu_apelli from personas a where a.nrodoc='$nrodocu'";
$datos= array(); 
 $res=  $this->DB1->query(  $sql );
  $datos= $res->result_array();
  return $datos;
...
$sql=" select  f.edad from fallecidos.fallecidos f  where f.nrodoc= '$nrodoc' ";
...
$sql= "select ent_sal_destino,ent_sal_fecha, ent_sal_medio, ent_sal_chapa, ent_sal_observ , tipo_mov  
from deportivo.ent_sal_pais where nrodoc='$nrodocu'" ; 
$query=  $this->DB1->get_where('deportivo.ent_sal_pais', array('nrodoc' => $nrodocu));
